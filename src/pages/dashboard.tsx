...
import { useRoqPlatformUserProfiles, usePriceTrendCount, useWalletCount, useUserProfileCount } from 'lib/roq';

import { PriceTrendListPage as ShowCaseList } from 'pages/price-trends';

function DashboardPage() {
  const { session } = useSession();

  const priceTrendStat = usePriceTrendCount();

  const walletStat = useWalletCount();

  const userProfileStat = useUserProfileCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Price Trends',
      result: priceTrendStat?.data,
      isLoading: priceTrendStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Wallets',
      result: walletStat?.data,
      isLoading: walletStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Profiles',
      result: userProfileStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`price-trends`} passHref legacyBehavior>
                  <Button
